...
''' 
Minimum spanning tree instance generator.

Usage: mstgen n_nodes n_edges cost_min cost_max [% high-cost [seed]]
...
Output: DIMACS_like format
        ...
        Line starting with "p" contains the string "mst", desgnating the 
...
if len(sys.argv) < 5:
    print( "Usage: mstgen n_nodes n_arcs cost_min cost_max [% high-cost [seed]]")
...
if n_arcs < n_nodes - 1 or n_arcs > n_nodes * (n_nodes - 1) / 2:
...
print( "p mst ", n_nodes, n_arcs)

# No DIMACS-format node descriptors, because MST does not have sources or sinks

# Build a tree.  Add nodes in order 0, 1, ..., n_nodes - 1, choosing
# the other endpoint of the added arc at random from nodes already in
# the tree.
...
for i in range(n_arcs * hicost//100):
    cost[i] = cost_max
...
for j in range(n_arcs):
    print( "e ", arcs[j][0], arcs[j][1], cost[j])
